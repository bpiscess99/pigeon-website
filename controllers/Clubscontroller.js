...
export const registercontroller = async (req, res) => {
    try {
        const { name, cname, email, password, role } = req.body;

        if (!name) return res.send({ message: 'Name is required' });
        if (!cname) return res.send({ message: 'Club name is required' });
        if (!email) return res.send({ message: 'Email is required' });
        if (!password) return res.send({ message: 'Password is required' });
        if (typeof role === 'undefined') return res.send({ message: 'Role is required' });
    ...
    try {
        ...
        res.status(200).send({
            ...
            user: {
                ...
                email: user.email,
                role: user.role,
             //   id: user._id, // Include id in the response
